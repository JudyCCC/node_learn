      ...
      case 'del':
        db.query(`SELECT * FROM custom_evaluation_table WHERE ID=${req.query.id}`,(err,data)=>{
          if(err){
            ...
            if(data.length==0){
              res.status(404).send('no custom evaluation').end();
            }else{
              fs.unlink('./29_static/upload/' + data[0].src,(err)=>{
                if(err){
                  console.error(err);
      ...
      case 'mod':
        db.query(`SELECT * FROM custom_evaluation_table WHERE ID=${req.query.id}`,(err,data)=>{
          if(err){
            ...
            res.status(500).send('no this ecaluation').end();
          }else{
          ...
          if(req.body.mod_id){    //修改
            ...
            db.query(`SELECT * FROM custom_evaluation_table WHERE ID=${req.body.mod_id}`,(err,data)=>{
              if(err){
                ...
                res.status(500).send('old file not found').end();
              }else{
                fs.unlink('29_static/upload/' + data[0].src,(err)=>{
                  if(err){
                    console.error(err);
